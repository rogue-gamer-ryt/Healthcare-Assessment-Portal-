...
{
	creator=(String)session.getAttribute("name");
	practice=(String)session.getAttribute("pid");
}
else
{
...
<head>
<meta http-equiv="refresh" content="0; url=manage_practice.jsp">
</head>
   ...
   { 
      ...
      while ( i.hasNext () ) 
      {
         ...
         if(fi.isFormField())
         {
             ...
             if(ntype.equals("pname"))
             {
                 name=(String)fi.getString();
             }
             else if(ntype.equals("pcity"))
             {
         ...
         String sql="Update root_practice set name='"+name+"',city='"+city+"',logo='"+path+"' where practiceID='"+practice+"'";
         prs=connection.prepareStatement(sql);
         prs.executeUpdate();
         String action="Edit ";
         action=action.concat(name);
         sql="Insert into root_log(action,actioncreator,actionpage) values('"+action+"','"+creator+"','Practice')";
         prs=connection.prepareStatement(sql);
         prs.executeUpdate();
      	} 
      	}
