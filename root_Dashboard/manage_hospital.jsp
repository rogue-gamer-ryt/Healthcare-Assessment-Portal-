            ...
            <div class="container-fluid">
                ...
                <div class="row">
                   <div class="push-s7 push-m10 push-l10 col s4 m3 l2">
                    <a href="create_hospital.jsp" class="waves-effect waves-light btn">Add</a>
                  </div>
                   <div class="push-s7 push-m10 push-l6 col s4 m3 l2">
                    <a target="_blank" href="audittrials.jsp?str=Hospital" class="waves-effect waves-light btn">Audit Trials</a>
                  </div> 
                </div>
                <div class="row">
                        ...
                        <table class="centered striped">
							...
							<% 
							  try
							    ...
							    String sql = "Select root_hospital.*,root_practice.* from root_hospital INNER JOIN root_practice ON root_practice.practiceID=root_hospital.practiceID";
							...
							%>
                              <tr>
                                <td><%=f++ %></td>
                                <td><%=resultSet.getString("root_practice.name") %></td>
                                <td><%=resultSet.getString("root_hospital.name") %></td>
                                <td><%=resultSet.getString("root_hospital.city") %></td>
                                <td><%=resultSet.getString("root_hospital.contact") %></td>
                                <td><%=resultSet.getString("root_hospital.address") %></td>
								<td><a class="waves-effect waves-light btn red"  onclick="setValue1(<%=resultSet.getString("hospitalID") %>)" href="#modal1"> Edit </a></td>
                                <td><a class="waves-effect waves-light btn red"  onclick="setValue2(<%=resultSet.getString("hospitalID") %>)" href="#modal2"> Enable/Disable</a></td>
                                <td><%=resultSet.getString("root_hospital.status") %></td>
                                <td><%=resultSet.getString("root_hospital.creator") %></td>
                                <td><%=resultSet.getString("root_hospital.startdate") %></td>
                                <td><%=resultSet.getString("root_hospital.expirydate") %></td>
                              </tr>
							 <%
								}
								}
	...
	if(val==1)
		{
			...
			document.editForm.action="edit_hospital.jsp";
			document.editForm.submit();
		}
	if(val==2)
		{
			document.toggleForm.action="POST";
			document.toggleForm.action="toggle_hospital.jsp";
